...
static bool callFunction(ObjFunction *function, int argCount) { return call((Obj *) function, function, argCount); }
...
#define READ_SHORT() (frame->ip += 2, (uint16_t)((frame->ip[-2] << 8) | frame->ip[-1]))
    ...
    // #define READ_CONSTANT() (((ObjClosure *) (frame->function))->function->chunk.constants.values[READ_BYTE()])

    Value READ_CONSTANT()
    {
        ObjFunction *fun = NULL;
        switch (frame->function->type) {
            case OBJ_FUNCTION:
                fun = (ObjFunction *) frame->function;
                break;
            case OBJ_CLOSURE:
                fun = ((ObjClosure *) frame->function)->function;
                break;
            default:
                runtimeError("Expect an Obj of type Function or Closure, get %d instead!", frame->function->type);
        }
        return fun->chunk.constants.values[READ_BYTE()];
    }
...
#define BINARY_OP(valueType, op)                                                                                       \
    ...
    for (;;) {
        ...
        printf("\n");
        disassembleInstruction(&((ObjClosure *) (frame->function))->function->chunk,
                               (int) (frame->ip - ((ObjClosure *) (frame->function))->function->chunk.code));
#endif
